...
user_emotion — общий эмоциональный тон клиента в диалоге:
  Позитив      — клиент доволен, благодарит, настроен дружелюбно
  Нейтральный  — деловой тон, без выраженных эмоций
  Негатив      — раздражение, недовольство, претензии, злость

resolution_status — итог диалога с точки зрения клиента:
  Решено        — проблема закрыта, клиент получил ответ или действие
  Не решено     — проблема осталась открытой: оператор не помог, дал отписку, \
диалог оборвался без результата
  Частично      — часть вопроса решена, часть нет
  Эскалация     — вопрос передан на другой уровень или другому специалисту

business_signal — главный бизнес-сигнал из диалога (выбери ОДИН наиболее важный):
  Логистика      — проблемы с доставкой, задержки, потери
  ПВЗ            — проблемы на пункте выдачи
  Финансы        — возвраты, списания, оргсборы
  Продукт        — баги, UX-проблемы, сбои платформы
  Организаторы   — жалобы на организаторов закупок
  Товар          — брак, несоответствие, пересорт
  Поддержка      — проблема с качеством работы поддержки
  Рост           — пожелания, партнёрство, позитивные сигналы
  Нет сигнала    — служебный диалог, тест, нерелевантно

agent_quality_label — оценивай ТОЛЬКО качество ответа оператора, не результат \
(оператор не виноват в задержке доставки, но виноват если груб или не помог):
  Отлично         — вежлив, чёткий ответ, решил или направил правильно
  Хорошо          — корректен, но есть небольшие недочёты
  Удовлетворительно — ответил формально, мало помог
  Плохо           — груб, проигнорировал, дал неверную информацию
