...
user_emotion: Позитив | Нейтральный | Негатив
Определи общую эмоцию пользователя по всему диалогу, а не только по последнему сообщению.

Варианты:
- Негатив — есть жалобы, раздражение или недовольство
- Нейтральный — без выраженных эмоций, пользователь удовлетворён (например: "спасибо", "понял")
- Позитив — выраженная благодарность или высокий уровень удовлетворённости

resolution_status: Решено | Не решено | Частично | Эскалация
Определи итог обработки обращения пользователя.

Выбери только один вариант:
- Решено — проблема полностью решена в диалоге, пользователь получил конкретный результат
- Не решено — проблема не решена, нет результата или ответа по существу
- Частично — дана информация или промежуточный ответ, но проблема не закрыта
- Эскалация — проблема передана другому сотруднику или отделу, решение вне текущего диалога

Важно: не считай проблему решённой, если:
- оператор только пообещал разобраться
- проблема передана дальше
- пользователь должен ждать

business_signal: Логистика | ПВЗ | Финансы | Продукт | Организаторы | Товар | Поддержка | Рост | Нет сигнала
Определи, к какой зоне ответственности относится проблема. Выбери один вариант:
- Логистика — проблемы доставки и перемещения заказов
- ПВЗ — проблемы в пункте выдачи (приём, выдача, сканирование)
- Финансы — оплата, возвраты, деньги
- Продукт — ошибки сайта, интерфейса, функционала
- Организаторы — проблемы с закупками или поставщиками
- Товар — качество или несоответствие товара
- Поддержка — проблема в работе сотрудника
- Рост — предложения и идеи
- Нет сигнала — если проблемы нет (консультация или благодарность)

agent_quality_label: Отлично | Хорошо | Удовлетворительно | Плохо
agent_quality_score: число от 0 до 100, соответствует label:
- Отлично → 90–100
- Хорошо → 70–89
- Удовлетворительно → 40–69
- Плохо → 0–39

Оцени качество ответа сотрудника. Выбери один вариант:
- Отлично — сотрудник дал точное, понятное и полезное решение или ответ
- Хорошо — ответ в целом полезный, но не идеальный (мало деталей или не полностью раскрыт)
- Удовлетворительно — ответ частичный, без чёткого решения или с неопределённостью
- Плохо — ответ не помогает, нерелевантен или не решает проблему

Важно: не оценивай результат (решена ли проблема). Оцени только качество ответа сотрудника.

churn_risk_score: вероятность оттока клиента, число от 0.0 до 1.0:
- 0.0–0.2 — клиент доволен или нейтрален, признаков оттока нет
- 0.3–0.5 — есть недовольство, но не критичное
- 0.6–0.8 — явное раздражение, проблема не решена
- 0.9–1.0 — угрожает уйти, сравнивает с конкурентами, сильный негатив

needs_escalation: 0 или 1.
Поставь 1, если выполняется хотя бы одно условие:
- resolution_status = Эскалация
- churn_risk_score >= 0.8
- клиент явно требует руководство, компенсацию или возврат денег
...
Тип источника обращения (source_type) — кто написал в чат:

ПВЗ        — если выполняется хотя бы одно условие:
             • в имени посетителя есть слово «ПВЗ»
             • имя посетителя содержит название города или адрес (например: «Жигулевск Моркваши», «Тольятти Автозаводская», «Самара Победы 20»)
             • имя выглядит как географическое название, а не имя человека
Сотрудник  — если по контексту диалога очевидно что пишет внутренний сотрудник \
(служебные вопросы, знает внутренние процессы, пишет от лица компании)
Клиент     — обычный покупатель (по умолчанию)

Формат ответа — строго JSON, без пояснений:
{{
  "source_type": "одно из: ПВЗ | Сотрудник | Клиент",
  ...
  "user_problem_summary": "краткое описание проблемы и итога её решения, 1-3 предложения. Обязательно укажи: (1) суть обращения со всеми идентификаторами (номер заказа, посылки, артикул, закупка и др.); (2) что сделал оператор и чем завершился диалог (решено/не решено/передано).",
  "user_emotion": "одно из: Позитив | Нейтральный | Негатив",
  "churn_risk_score": 0.0,
  "resolution_status": "одно из: Решено | Не решено | Частично | Эскалация",
  "needs_escalation": 0,
  "agent_quality_score": 75,
  "agent_quality_label": "одно из: Отлично | Хорошо | Удовлетворительно | Плохо",
  "agent_quality_comment": "что сделал хорошо/плохо, 1 предложение",
  "business_signal": "одно из: Логистика | ПВЗ | Финансы | Продукт | Организаторы | Товар | Поддержка | Рост | Нет сигнала",
...
def call_ai(prompt: str, max_tokens: int = None, timeout: int = None, model: str = None) -> Optional[str]:
    ...
    _model = model if model is not None else AI_MODEL
...
def parse_response(ai_text: str) -> Optional[dict]:
    ...
    try:
        ...
        return {
            "source_type":           str(data.get("source_type") or "Клиент"),
            "contact_reason":        str(data.get("contact_reason") or "Другое"),
            "category":              str(data.get("category") or "Не определено"),
            ...
            "resolution_status":     str(data.get("resolution_status") or "Не решено"),
            ...
            "business_signal":       str(data.get("business_signal") or "Нет сигнала"),
