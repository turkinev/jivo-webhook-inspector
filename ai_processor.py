...
запрос_действия — просит сделать что-то конкретное («верните деньги», «отмените заказ», «исправьте», «перенесите»)
...
{{
  "contact_reason": "одно из: жалоба | консультация | проблема | статус | запрос_действия | уточнение | благодарность | другое",
