...
ВАЖНО: между каждым разделом ставь ровно одну строку-разделитель: ===SPLIT===
...
===SPLIT===
(для каждого отдела с обращениями — отдельный блок, разделённый ===SPLIT===)
...
def main():
    ...
    parts = [p.strip() for p in report_text.split("===SPLIT===") if p.strip()]
    if not parts:
        parts = [report_text]

    # Первый блок получает заголовок отчёта
    parts[0] = f"{header}\n\n{parts[0]}"

    if args.dry_run:
        print(f"\n=== ОТЧЁТ ({len(parts)} блоков) ===")
        for i, p in enumerate(parts, 1):
            print(f"\n--- блок {i} ---\n{p}")
        return

    # Telegram: отправляем всё как одно сообщение (4096 лимит)
    send_telegram("\n\n".join(parts))

    # Mattermost: каждый блок — отдельное сообщение
    for i, part in enumerate(parts, 1):
        send_mattermost(part)
        if len(parts) > 1:
            print(f"  → блок {i}/{len(parts)} отправлен")
